...
The palette is rooted in a deep, nocturnal foundation, utilizing the tension between `primary` (#6366F1) and `secondary` (#64748B) to signify intelligence and kinetic energy. The overall neutral background is a very dark, muted `#0B0e14`.
...
- **Base Layer:** `surface` (#0B0e14, derived from `neutral_color_hex`).
...
- **Signature Gradients:** For primary CTAs and hero data points, use a linear gradient: `primary` (#6366F1) to `primary-container` (#8083ff). This prevents the UI from feeling "flat" or "default."
...
*   **Display & Headlines (Manrope):** Chosen for its modern, geometric character, aligning with the `headline_font` token. Use `display-lg` for "Big Numbers" (e.g., Win Probabilities) to create an authoritative, magazine-like feel.
*   **Body & Labels (Inter):** The workhorse for high-density data, aligning with the `body_font` and `label_font` tokens. Inter’s tall x-height ensures readability in complex stat tables.
...
- For cards, use `surface-container-high` and a subtle `0.5rem` (lg) or `0.75rem` (xl) corner radius. The `roundedness` token is set to `1` for a subtle roundedness across the system.
...
- Use a `tertiary` (#F59E0B) accent for "Live" or "Critical" data chips to provide a sophisticated gold pop against the deep navy.
...
### Do:
- **Do** use whitespace as a structural element. If an interface feels "empty," increase the typography scale of the headline rather than adding a border. The `spacing` token is set to `2` for a normal density.
...
### Don't:
- **Don't** use pure black (#000). Always use the `surface` (#0B0e14) for true premium depth.
